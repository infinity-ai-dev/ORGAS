...
def _build_subgraph_initial_state(document: dict) -> dict:
...
def _execute_subgraph_safe(subgraph_name: str, document: dict) -> dict:
    """
    ...
    try:
        ...
        initial_state = _build_subgraph_initial_state(document)
...
def _process_single_document(state: dict) -> dict:
    ...
    # Executar sub-grafo
    exec_result = _execute_subgraph_safe(detected_type, document)
    elapsed = time.perf_counter() - start
...
def _run_single_analysis(state: dict) -> dict:
    ...
    exec_result = _execute_subgraph_safe(analysis_type, document)
    elapsed = time.perf_counter() - start
