...
1. Google Gemini 2.5 Pro (Principal) ← PRINCIPAL
...
def get_gemini_model(
    ...
    """
    Retorna modelo Google Gemini 2.5 Pro.
    ...
    logger.info(f"📱 Carregando Gemini 2.5 Pro (temp={temp}, max_tokens={max_tok})")
...
def get_model_with_fallback(
    ...
    Ordem:
    1. Google Gemini 2.5 Pro (Principal)
    2. OpenAI GPT-4 Turbo (Fallback 1)
    3. Grok/xAI (Fallback 2)
    ...
    for provider in settings.llm_fallback_order:
        try:
            if provider == "gemini":
                logger.info("✅ Tentando Gemini 2.5 Pro (principal)...")
                return get_gemini_model(temperature, max_tokens)
