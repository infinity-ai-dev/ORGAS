...
async def fetch_generic_data(state: AgentState) -> dict[str, Any]:
    ...
    Generic usa apenas database (contexto do usuário).
    RAG desabilitado para melhorar performance e evitar erros de embedding.
    """
    logger.info("🔍 Nó 2: Buscando dados para responder (via módulo)...")

    # Chamar módulo apenas com fonte database
    result = await fetch_data_module(
        state,
        domain="generic",
        sources=["database"],  # Apenas contexto do usuário, sem RAG
    )
